...
-- Adds first-class project collaboration for team members
-- Projects can be created by CEO, manager, or tech lead
-- Members see the full team progress but can only update their own task
...
-- ---------------------------------------------------------------------------
-- overall_completion auto-update trigger
-- Recalculates project overall_completion whenever a member row changes
-- ---------------------------------------------------------------------------
...
-- projects: readable by any member of that project, or the project creator
...
-- projects: insert/update/delete only by authenticated (CEO/manager creates, no member-only restriction at DB level)
...
-- project_members: members can read all rows for projects they belong to
...
-- project_members: insert by authenticated (manager/CEO assigns)
CREATE POLICY "project_members_insert" ON public.project_members
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- project_members: employees can only update their own row
CREATE POLICY "project_members_update" ON public.project_members
  FOR UPDATE USING (profile_id = auth.uid());

-- project_members: only creator of project or the member themselves can delete
...
-- notifications: only the recipient can read or update their own notifications
