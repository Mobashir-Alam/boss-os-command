...
const PriorityCard = ({ priority, index, linkedTasks = [] }: PriorityCardProps) => {
  ...
  const allTasksDone = linkedTasks.length > 0 && linkedTasks.every((t) => t.status === "completed");
  ...
  return (
    ...
    >
      ...
      {linkedTasks.length > 0 && (
        <div className="mb-5">
          <TaskList tasks={linkedTasks} title="Linked Tasks" />
        </div>
      )}
      ...
      {allTasksDone && (
        <div className="mb-5">
          <ResolutionPrompt
            issueLabel={`${priority.startupName} — ${priority.tag}`}
            onResolve={() => setStatus("done")}
          />
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        <IssueTaskFlow
          linkedIssueId={priority.id}
          linkedStartupId={priority.startupId}
          defaultTitle={priority.mfoSuggestion}
        />
